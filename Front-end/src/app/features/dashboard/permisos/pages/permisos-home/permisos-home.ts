...
interface EstadoSolicitud {
  id: number;
  nombre: 'Pendiente' | 'Aprobado' | 'Rechazado' | 'Anulado';
}
...
export class PermisosHome {
  ...
  // ── Datos simulados ──────────────────────────────────────────
  usuarios: Usuario[] = [
    { id: 1, nombre: 'Laura', apellido: 'Mendoza', correo: 'laura.mendoza@empresa.com' },
    { id: 2, nombre: 'Carlos', apellido: 'Quispe', correo: 'carlos.quispe@empresa.com' },
    { id: 3, nombre: 'María', apellido: 'Torres', correo: 'maria.torres@empresa.com' },
    { id: 4, nombre: 'Diego', apellido: 'Vargas', correo: 'diego.vargas@empresa.com' },
    { id: 5, nombre: 'Sofía', apellido: 'Rojas', correo: 'sofia.rojas@empresa.com' },
  ];

  tiposSolicitudPermiso: TipoSolicitudPermiso[] = [
    { id: 1, nombre: 'Permiso personal', descripcion: 'Ausencia por asuntos personales no especificados.', requiereDocumento: false, remunerado: true, estado: 'Activo' },
    { id: 2, nombre: 'Permiso médico', descripcion: 'Ausencia por cita o tratamiento médico.', requiereDocumento: true, remunerado: true, estado: 'Activo' },
    { id: 3, nombre: 'Permiso familiar', descripcion: 'Ausencia por situaciones de carácter familiar.', requiereDocumento: false, remunerado: true, estado: 'Activo' },
    { id: 4, nombre: 'Permiso por estudios', descripcion: 'Ausencia por actividades académicas o de formación.', requiereDocumento: true, remunerado: false, estado: 'Activo' },
    { id: 5, nombre: 'Permiso administrativo', descripcion: 'Ausencia para trámites administrativos o legales.', requiereDocumento: true, remunerado: false, estado: 'Inactivo' },
  ];
  ...
  // GETTERS
  // ─────────────────────────────────────────────────────────────
  get totalSolicitudes(): number { return this.solicitudesPermiso.length; }

  get totalPendientes(): number {
    return this.solicitudesPermiso.filter(s => this.obtenerEstadoSolicitud(s.estadoSolicitudId)?.nombre === 'Pendiente').length;
  }

  get totalAprobadas(): number {
    return this.solicitudesPermiso.filter(s => this.obtenerEstadoSolicitud(s.estadoSolicitudId)?.nombre === 'Aprobado').length;
  }

  get totalRechazadas(): number {
    return this.solicitudesPermiso.filter(s => this.obtenerEstadoSolicitud(s.estadoSolicitudId)?.nombre === 'Rechazado').length;
  }

  get totalAnuladas(): number {
    return this.solicitudesPermiso.filter(s => this.obtenerEstadoSolicitud(s.estadoSolicitudId)?.nombre === 'Anulado').length;
  }
  ...
  get solicitudesFiltradas(): SolicitudPermiso[] {
    ...
    return this.solicitudesPermiso.filter(s => {
      ...
      const coincideTexto = !termino || nombreCompleto.includes(termino) || tipoNombre.includes(termino) || motivo.includes(termino);
      const coincideTipo = !this.filtroTipo || s.tipoSolicitudPermisoId === Number(this.filtroTipo);
      const coincideEstado = !this.filtroEstado || s.estadoSolicitudId === Number(this.filtroEstado);
  ...
  obtenerIdEstadoPorNombre(nombre: string): number {
  ...
  guardarSolicitud(): void {
    if (!this.formUsuarioId) { this.mostrarMensaje('El campo Usuario es obligatorio.', 'error'); return; }
    if (!this.formTipoId) { this.mostrarMensaje('El campo Tipo de permiso es obligatorio.', 'error'); return; }
    if (!this.formFechaI) { this.mostrarMensaje('La Fecha inicio es obligatoria.', 'error'); return; }
    if (!this.formFechaF) { this.mostrarMensaje('La Fecha fin es obligatoria.', 'error'); return; }
    if (!this.formMotivo.trim()) { this.mostrarMensaje('El campo Motivo es obligatorio.', 'error'); return; }
    if (this.formFechaI > this.formFechaF) { this.mostrarMensaje('La fecha inicio no puede ser mayor que la fecha fin.', 'error'); return; }
  ...
  editarSolicitud(solicitud: SolicitudPermiso): void {
    if (!this.puedeGestionarse(solicitud)) {
      this.mostrarMensaje('Solo se pueden editar solicitudes en estado Pendiente.', 'error');
      return;
    }
  ...
  cambiarEstado(id: number, nuevoEstadoNombre: string, observacion = ''): void {
    const nuevoEstadoId = this.obtenerIdEstadoPorNombre(nuevoEstadoNombre);
    this.solicitudesPermiso = this.solicitudesPermiso.map(s =>
      s.id === id ? { ...s, estadoSolicitudId: nuevoEstadoId, observacionRevision: observacion } : s
    );
  }

  aprobarSolicitud(id: number): void {
    const s = this.solicitudesPermiso.find(x => x.id === id);
    if (!s || !this.puedeGestionarse(s)) return;
    this.cambiarEstado(id, 'Aprobado');
    this.mostrarMensaje('Solicitud aprobada.', 'ok');
    if (this.solicitudDetalle?.id === id) {
      this.solicitudDetalle = this.solicitudesPermiso.find(x => x.id === id) ?? null;
    }
  }
...
abrirModalRevision(id: number, accion: 'Rechazado' | 'Anulado'): void {
  ...
  if (!solicitud || !this.puedeGestionarse(solicitud)) {
    this.mostrarMensaje('Solo se pueden gestionar solicitudes en estado Pendiente.', 'error');
    return;
  }
...
cerrarModalRevisionPorOverlay(event: MouseEvent): void {
  if ((event.target as HTMLElement).classList.contains('modal-revision-overlay')) {
    this.cerrarModalRevision();
  }
}

confirmarRevision(): void {
  if (!this.solicitudRevision || !this.accionRevision) return;
  ...
  if (this.solicitudDetalle?.id === this.solicitudRevision.id) {
    this.solicitudDetalle =
      this.solicitudesPermiso.find(x => x.id === this.solicitudRevision!.id) ?? null;
  }
...
get textoBotonRevision(): string {
  ...
  aprobarDesdeModal(): void {
    if (this.solicitudDetalle) {
      ...
      this.solicitudDetalle = this.solicitudesPermiso.find(x => x.id === this.solicitudDetalle!.id) ?? null;
    }
  }
